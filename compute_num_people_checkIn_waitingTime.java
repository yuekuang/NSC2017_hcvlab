...
public class compute_num_people_checkIn_waitingTime{
    ...
    public static void main(String[] args) throws Exception {
        ...
        // name of output file 
        File outputFile = new File("ppl_time_MWF_wait_checkIn_avgwaitingTime.csv");
        ...
        // make the input csv file to 2D array
        ...
        for(int j=0; j< time.size(); j++){
            ...
            ArrayList<Integer> arrival_intake = new ArrayList<Integer>();
            for(int i=0; i<lineArray.length; i++){
                ...
                if(comp1 <= 0 && comp2 > 0) {
                    ...
                    long waitingTime_checkIn = (intake_time.getTime() - arrival_time.getTime())/ (60 * 1000) % 60;
                    
                    // convert to integer
                    int waitingTime_checkIn_int = (int)waitingTime_checkIn;
                    
                    // add the waiting time values to arraylist
                    arrival_intake.add(waitingTime_checkIn_int);
                }
            }

            // calculate sum
            int sum = 0;
            for(int k=0; k<arrival_intake.size(); k++){
                sum = sum + arrival_intake.get(k); 
            }
